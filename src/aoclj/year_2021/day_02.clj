...
(defrecord Submarine [horizon depth]
  ICommand
  (forward [{:keys [horizon depth]} x]
    (->Submarine (+ horizon x)
                 depth))
  (down [{:keys [horizon depth]} x]
    (->Submarine horizon
                 (+ depth x)))
  (up [{:keys [horizon depth]} x]
    (->Submarine horizon
                 (- depth x))))
...
(defrecord SubmarineWithAim [horizon depth aim]
  ...
  (forward [{:keys [horizon depth aim]} x]
    (->SubmarineWithAim (+ horizon x)
                        (+ depth (* aim x))
                        aim))
  (down [{:keys [horizon depth aim]} x]
    (->SubmarineWithAim horizon
                        depth
                        (+ aim x)))
