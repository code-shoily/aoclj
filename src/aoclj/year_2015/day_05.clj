...
(defn enough-vowels?
  [s] (>= (count (filter #{\a \e \i \o \u} s)) 3))

(defn twice-in-a-row? [s]
...
(defn has-non-overlapping-twins?
  ...
  (->> s
       ...
       (reduce (fn [[pairs last-pair] current-pair]
                 (cond
                   (= last-pair current-pair) [pairs nil]
                   (pairs current-pair) (reduced true)
                   :else [(conj pairs current-pair) current-pair]))
               [#{} nil])
       true?))

(def is-nice-v1? (check-niceness-for [enough-vowels? twice-in-a-row? has-no-naughty-strings?]))

(def is-nice-v2? (check-niceness-for [has-non-overlapping-twins? has-sandwitched-letters?]))

(def parse str/split-lines)

(defn part-1 [input] (->> input (filter is-nice-v1?) count))

(defn part-2 [input] (->> input (filter is-nice-v2?) count))

(def solve (utils/generic-solver part-1 part-2 parse))

(comment "<Explore>"
         (def input-data
           (utils/read-input-data 2015 5))
