...
(defn make-password-policy [line]
  (let [[_ lo hi ch pwd]
        (re-matches #"(\d+)-(\d+) (.): (.+)" line)]
    (->PasswordPolicy
     (Integer/parseInt lo) (Integer/parseInt hi) (first ch) pwd)))

(defn valid-by-frequency?
  [{:keys [lo hi ch pwd]}]
  (let [freq (get (frequencies pwd) ch 0)]
    (<= lo freq hi)))
...
(defn check-with [f] #(->> % (filter f) count))
(defn parse
  [input]
  (map make-password-policy
